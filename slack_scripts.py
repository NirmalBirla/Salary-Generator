...
def messages_in_date_range(all_slack_messages, users_list, start_date, end_date):
    # Filter messages based on date range
    start_date = datetime.strptime(start_date, "%Y-%m-%d").date() - timedelta(days=30) # We are getting past two months leave applications.
    ...
    filtered_messages.sort(key=lambda x: (x["name"].lower(), datetime.strptime(x["date"], "%Y-%m-%d")))
...
def slack_api_run(file_name, start_date, end_date):
    try:
        ...
        filtered_messages = messages_in_date_range(all_slack_messages, users_list, start_date, end_date)
